...
void Ds1302Write(uchar addr, uchar dat)
{
	...
	SCLK = 0;         /* 先将时钟线拉低 */
	_nop_();
	RST = 1;          /* 然后RST(CE)拉高，开始通信 */
	_nop_();

	for (n=0; n<8; n++)    /* 发送8位地址指令 */
	{
		DSIO = addr & 0x01;     /* 数据从低位开始 */
		addr >>= 1;
		SCLK = 1;               /* 上升沿DS1302读取数据 */
		_nop_();
		SCLK = 0;
		_nop_();
	}
 ...
 * 输入 ：无
 * 输出 ：无
 * 说明 ：判断CH位，如果时钟未运行则写入默认时间
 */
void Ds1302Init()
{
	uchar n;
	uchar sec;
	sec = Ds1302Read(0x81);      /* 读取秒寄存器检查CH位 */
	if(!(sec & 0x80))            /* CH=0表示时钟正在运行，无需初始化 */
		return;                  /* 首次上电，写入默认时间 */
